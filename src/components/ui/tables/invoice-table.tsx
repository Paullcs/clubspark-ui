        ...
        <TableFooter className="border-0 bg-transparent font-normal">
          {summary.map((row, i) => (
            <TableRow key={i} className="border-0 hover:bg-transparent">
              {/* Desktop: label spans the first three columns so it sits next to the amount */}
              <TableCell
                colSpan={3}
                className={cn(
                  "hidden border-0 py-2 pr-3 text-right align-middle text-sm sm:table-cell",
                  i === 0 && "border-t border-border pt-4",
                  row.bold ? "font-semibold text-foreground" : "font-normal text-muted-foreground"
                )}
              >
                {row.label}
              </TableCell>
              {/* Mobile: label is its own column, amount sits to the right */}
              <TableCell
                className={cn(
                  "border-0 py-2 pr-3 text-left align-middle text-sm sm:hidden",
                  i === 0 && "border-t border-border pt-4",
                  row.bold ? "font-semibold text-foreground" : "font-normal text-muted-foreground"
                )}
              >
              ...
              <TableCell className={cn(
                "border-0 py-2 pl-3 text-right align-middle text-sm",
                i === 0 && "border-t border-border pt-4",
